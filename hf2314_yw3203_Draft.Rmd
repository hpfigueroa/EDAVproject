...
```{r initial_plot_load}
gvl0 <- ggplot(dfl, aes(Time_Stamp, Actual_Load)) +
  geom_line() +
  geom_smooth(method = "loess", span = .5, se = FALSE) +
  ggtitle("Initial Visualization of Load time series From 2014/11/1 to 2018/10/31") +
  xlab("Date") +
  ylab("Load (MWh)")
#+
...
gvl0
```
```{r initial_plot_price}
gvp0 <- ggplot(dfp, aes(Time_Stamp, Actual_Price)) +
  geom_line() +
  geom_smooth(method = "loess", span = .5, se = FALSE) +
  ggtitle("Initial Visualization of Price time series From 2014/11/1 to 2018/10/31") +
  xlab("Date") +
  ylab("Load (MWh)")
#+
#  scale_x_date(date_breaks = "1 month", date_labels = "%b %y")
